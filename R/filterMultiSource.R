...
#'
#' @param predict.cont data frame, predicted contamination data
...
#' @usage filterMultiSources(filename,output_path,VAFdata,uniq_both)
#'
    ...
    if(nrow(current) ==2){
      ...
      for(j in 1:n.pair){
        ...
        s2 <- pair.file[2,j]
          ...
          if(method1){
            s1.target$dist <-  sqrt( (s1.target$source*cont1/100 - s1.target$target)^2 * log10(s1.target.min.cov))
            s2.target$dist <-  sqrt( (s2.target$source*cont2/100 - s2.target$target)^2 * log10(s2.target.min.cov))
            row.min.cov <-current.data[which(current.data$target_cov > minCov & current.data$s1_cov >minCov & current.data$s2_cov> minCov),'mutationID']
            n.targetOnly <- length(row.min.cov)
            d1 <- sqrt( (s1.s2.target.only.vaf[s1.s2.target.only.vaf$mutationID %in% row.min.cov,2]*cont1/100 - s1.s2.target.only.vaf[s1.s2.target.only.vaf$mutationID %in% row.min.cov,4])^2 * log10(min.cov.s1.target.only[s1.s2.target.only.vaf$mutationID %in% row.min.cov]))

            d2 <- sqrt( (s1.s2.target.only.vaf[s1.s2.target.only.vaf$mutationID %in% row.min.cov,3]*cont1/100 - s1.s2.target.only.vaf[s1.s2.target.only.vaf$mutationID %in% row.min.cov,4])^2 * log10(min.cov.s2.target.only[s1.s2.target.only.vaf$mutationID %in% row.min.cov]))
          }else{
            n.rep <- dim(s1.target)[1]
            s1.target$dist <-  sqrt( (s1.target$source/s1.target$target - rep(cont1/100,n.rep))^2 * log10(s1.target.min.cov))
            n.rep <- dim(s2.target)[1]
