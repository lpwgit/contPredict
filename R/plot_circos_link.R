...
#' @param contaminatedOnly logical, TRUE: only plot contaminated pairs (default: TRUE)
#' @param sameSubjectOnly logical, TRUE: only plot same subject pairs (default: FALSE)
#' @param allSamples character vector, sample ids
#' @param output_path character, directory
#'
#' @usage plot_circos_link (file,R,W,plotsize,titleStr,seg.lab.size,fig.file,
...
plot_circos_link <- function (file,R=300,W=30,plotsize=800,titleStr="",seg.lab.size=1,fig.file="circos.pdf", contaminatedOnly=TRUE,sameSubjectOnly=FALSE,allSamples=sampleID,output_path){
  ...
  if(any(c( sameSubject.flag, x2y.flag, y2x.flag,bothway.flag))){
    ...
    png(paste0(output_path,"/",fig.file), height = hsize, width = wsize,units = "in",res=200)
    par(mar=c(0, 0, 2, 0))
    ...
    # contamination + same subject in 1 circos
    #if(sameSubject.flag & contaminatedOnly){
    #}
    invisible(dev.off())
  }
